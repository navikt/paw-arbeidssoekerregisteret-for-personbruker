...
import { initialStyrkState, reducer } from '@/components/styrkloft/reducer';
import StyrkLoft from '@/components/styrkloft/styrk-loft';
...
export const PlaywrightTestVisStillinger: Story = {
    ...
    async play(context) {
        ...
        await userEvent.click(await canvas.getByRole('button', { name: 'Velg område' }));
        await userEvent.click(await screen.findByText('Oslo'));
        await userEvent.click(await screen.findByText('Velg alle'));
        await userEvent.click(await canvas.getByRole('button', { name: 'Velg område' })); // lukk områder
